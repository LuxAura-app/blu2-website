...
The provider abstraction (`api/lib/fulfillment/`) exists so this is additive,
not a rewrite. To add a provider (call it `newco`):

1. Add `'newco'` to the `FulfillmentProviderName` union in
   `api/lib/fulfillment/types.js`.
...
3. Register it in `api/lib/fulfillment/index.js`
...
5. `groupItemsByProvider` (`api/lib/fulfillment/grouping.js`) and
...
6. If the provider can push tracking/status updates, add
   ...
   `findOrderByProviderOrderId` in `api/lib/order-log.js`, and tolerate
   duplicate delivery.
