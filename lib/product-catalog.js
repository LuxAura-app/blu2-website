...
/**
 * Maps one catalog record's `variants[]` into the card shape `shop.html`
 * already renders (one card per variant — the no-new-UI option; see
 * docs/shop-architecture.md for why a size/color picker isn't built here).
...
function flattenProductToCards(product) {
  ...
  return variants.map((variant) => {
    ...
    return {
      ...
      image: (product.imageUrls && product.imageUrls[0]) || null,
